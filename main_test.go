...
func TestMainHelp(t *testing.T) {
	...
	if !strings.Contains(outputStr, expectedFibFlag) {
		t.Errorf("Expected output to contain %q, got %q", expectedFibFlag, outputStr)
	}
}

func TestMainFibonacci(t *testing.T) {
...
func TestMainFibonacciZero(t *testing.T) {
...
func TestMainFibonacciOne(t *testing.T) {
	cmd := exec.Command("go", "run", "main.go", "--fib=1")
	output, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("Command failed: %v", err)
	}
